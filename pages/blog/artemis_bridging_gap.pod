...
=head1 Artemis: Bridging gap between Vim and Neovim

L<vim-artemis|https://github.com/tani/vim-artemis>
...
This is where C<vim-artemis> steps in. It establishes a common interaction realm for
...
Initially, C<vim-artemis> needs to be loaded into your configuration file.

    local vimx = require("artemis")

Once done, C<vimx> can be used to interact with Vim and Neovim.

    vimx.cmd("colorscheme gruvbox")

Prefer to use C<vimx.cmd.colorscheme> instead of the code above? -- No issues.

    vimx.cmd.colorscheme("gruvbox")

Moreover, all means of calling C<vim.cmd> can be utilized.
We can employ C<keymap>, C<fn>, C<opt> among others. For instance,
...
For instance, C<{ 1 = 'a' }> is a lua table. Whether it's a dictionary or a list
...
Check the source code of the C<cast()> function for implementation details.

Even if Vim isn't your preference, C<vim-artemis> has utility for Neovim users.
It offers a superior C<fn> interface compared to Neovim's Lua API.
In VimL, C<namespace#function_name> is used to call a function defined in an autoload script.
In contrast, Neovim uses C<vim.fn['namespace#function_name']>, which is rather verbose.
C<vim-artemis> provides an improved interface.

    vimx.fn['namespace#function_name'] -- Neovim's Lua API
    vimx.fn.namespace.function_name     -- vim-artemis

This interface is supported through the use of the C<__index> metamethod.
It also facilitates chaining of deeper namespaces, such as C<namespace.subnamespace.function_name>.

    vimx.fn.namespace.subnamespace.function_name

To conclude, C<vim-artemis> establishes a common interface for integrating with Vim and Neovim.
