...
=head1 Quickfix is all you need
...
=item 1. Run external program defind as C<'makeprg'> option in Vim.

=item 2. Parse the output of the external program and convert it to a quickfix list.

=item 3. Display the quickfix list by C<:copen> command.
...
The C<'errorformat'> option is used to parse the output of the external program.
C<%f> is the file name, C<%l> is the line number, C<%c> is the column number,
C<%t> is the type of the error, which is either C<e> or C<w>, and C<%m> is the
error message.

Then, we set the C<'makeprg'> option to the external program and run the C<:make>.
The output of the external program is parsed and converted to a quickfix list.
Finally, we display the quickfix list by C<:copen> command.
